...
namespace IoTDeviceMonitor.Tests.ServicesTests;
...
public class DeviceServiceTests
{
    ...
    public void UpdateStatus_PersistsChange()
    {
        ...
        var reloaded = new DeviceService(new Logger(Path.Combine(Path.GetTempPath(), $"log-{Guid.NewGuid()}.txt")), new FileManager(path));
        Assert.Equal(DeviceStatus.Maintenance, reloaded.Devices[0].Status);
    }
    ...
    {
        var service = CreateService(out _);
        service.AddDevice(new Device { Id = "E1", Name = "Printer", IpAddress = "10.0.0.9" });
        service.AddDevice(new Device { Id = "F1", Name = "Scanner", IpAddress = "10.0.0.10" });
        service.AddDevice(new Device { Id = "G1", Name = "Monitor", IpAddress = "10.0.0.11" });
        service.AddDevice(new Device { Id = "H1", Name = "Keyboard", IpAddress = "10.0.0.12" });
        service.AddDevice(new Device { Id = "I1", Name = "Mouse", IpAddress = "10.0.0.13" });
        service.AddDevice(new Device { Id = "J1", Name = "Speaker", IpAddress = "10.0.0.14" });
        ...
        Assert.Equal("H1", ordered[1].Id); // Maintenance next
        Assert.Equal("I1", ordered[2].Id); // Online last
    }
}
